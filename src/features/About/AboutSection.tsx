...
const MusicChartIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
...
const CustomAngleTick: React.FC<any> = (props) => {
  ...
  const originalText = payload.value as string;
...
const AboutSection: React.FC<AboutSectionProps> = ({ designerName, aboutText }) => {
  const hobbyDetails = [
    {
      title: "玩音遊",
  ...
  const renderAboutTextWithBreaks = (text: string) => {
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <RadarChart cx="50%" cy="50%" outerRadius="75%" data={hobby.skills}>
                            ...
                            <Tooltip
                              ...
                              formatter={(value: number, name: string, props: any) => [`${value} / ${props.payload.fullMark}`, "熟練度"]}
                            />
                          </RadarChart>
                        </ResponsiveContainer>
                      </div>
